...
class OtpCheckViewController: UIViewController {
    ...
    private func setupUI() {
        ...
        otpStackView.leadingAnchor.constraint(equalTo: view.leadingAnchor, constant: 21).isActive = true
        otpStackView.trailingAnchor.constraint(equalTo: view.trailingAnchor, constant: -21).isActive = true
...
//        tf.layer.borderColor = UIColor(red: 247/255, green: 110/255, blue: 113/255, alpha: 1.0).cgColor
    ...
    @objc private func textFieldDidChange(_ textField: UITextField) {
        ...
        // Если текст удален (backspace)
        if text.isEmpty {
            updateTextFieldBorder(textField)
            checkOTPCompletion()
            
            // Переходим к предыдущему полю БЕЗ удаления его текста
            if textField.tag > 0 {
...
extension OtpCheckViewController: UITextFieldDelegate {
